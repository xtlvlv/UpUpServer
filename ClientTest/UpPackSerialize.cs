...
public class UpPackSerialize : SingleClass<UpPackSerialize>
{
    ...
    public void Serialize(UpNetPackage msg, ref Memory<byte> data)
    {
    ...
    public UpNetPackage? Deserialize(ReadOnlySequence<byte> data)
    {
        var headLen = GetHeadLen();

        if (data.Length < headLen)
        {
            Console.WriteLine($"data head length is not enough, need {headLen}, but {data.Length}");
            return null;
        }
        ...
        {
            Console.WriteLine($"data length is not enough, need {msgLen}, but {data.Length}");
            return null;
        }
